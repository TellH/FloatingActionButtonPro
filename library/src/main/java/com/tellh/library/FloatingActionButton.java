...
public class FloatingActionButton extends ImageButton {
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        ...
        if (attr != null) {
            try {
                ...
                mAnimationStype = attr.getInt(R.styleable.FloatingActionButton_fab_animation, 0);
            } finally {
                attr.recycle();
            }
        }
        ...
        if (mVisible != visible || force) {
            ...
            if (mAnimationStype == 0) {
                ...
                if (animate) {
                    ...
                    ViewHelper.setScaleX(this, scaleX);
                    ViewHelper.setScaleY(this, scaleY);
                }
            }
    ...
    public void attachToRecyclerView(@NonNull RecyclerView recyclerView,
                                     ScrollDirectionListener scrollDirectionlistener,
                                     RecyclerView.OnScrollListener onScrollListener, boolean isSensitiveScroll) {
        if (!isSensitiveScroll) {
            attachToRecyclerView(recyclerView, scrollDirectionlistener, onScrollListener);
        } else {
            RecyclerViewSensitiveScrollDetectorImpl scrollDetector = new RecyclerViewSensitiveScrollDetectorImpl();
            scrollDetector.setScrollDirectionListener(scrollDirectionlistener);
            scrollDetector.setOnScrollListener(onScrollListener);
            scrollDetector.setScrollThreshold(mScrollThreshold);
            recyclerView.addOnScrollListener(scrollDetector);
    ...
    private class RecyclerViewSensitiveScrollDetectorImpl extends RecyclerViewSensitiveScrollDetector {
        private ScrollDirectionListener mScrollDirectionListener;
        private RecyclerView.OnScrollListener mOnScrollListener;

        public RecyclerViewSensitiveScrollDetectorImpl() {
            super(height);
        }
        ...
        @Override
        void onMoved(int dis) {
            if (mAnimationStype == 0) {
                ViewHelper.setTranslationY(FloatingActionButton.this, dis);
            } else {
                float offset = height[0] == 0 ? 1.0f : 1 - (float) dis / (float) height[0];
                Log.d("TAG", "offset:" + offset);
                ViewHelper.setScaleY(FloatingActionButton.this, offset);
                ViewHelper.setScaleX(FloatingActionButton.this, offset);
            }
        }
